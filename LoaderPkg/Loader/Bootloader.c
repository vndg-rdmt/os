  ...
  )
{
  ...
  for (i = 0; i < MaxMode; ++i) {
    ...
    if (EFI_ERROR (Status)) {
      DEBUG ((DEBUG_ERROR, "JOS: Failed to obtain GOP Mode - %r\n", Status));
      return Status;
    }
    DEBUG ((DEBUG_INFO, "JOS: mode: %03d, height: %d, width: %d\n",
      i,
  ...
  */
  ...
  // JOS: mode: 007, height: 768, width: 1024
  Status = GraphicsOutput->SetMode (
    GraphicsOutput,
    7
  );
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "JOS: Failed to set GOP Mode - %r\n", Status));
    return Status;
  }
  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "JOS: Cannot close file - %r\n", Status));
    return Status;
  }
